...
The dashboard is split into two pages, reachable from the sidebar:
  * Create  -- ask the agent, draft fast reactions, curate a video.
  * Review  -- the approval queue, saved drafts, performance, agent trace.
A dark theme (see .streamlit/config.toml) is on by default.
...
def render_sidebar():
    with st.sidebar:
        ...
        # Live/dry-run switch. Seed from .env once, then this toggle owns it for
        # the session. We re-apply to os.environ on every rerun because
        # load_dotenv(override=True) at the top would otherwise reset it to the
        # .env value, and the publisher reads LINKEDIN_DRY_RUN from the
        # environment at post time.
        if "linkedin_live" not in st.session_state:
            st.session_state.linkedin_live = (
                os.getenv("LINKEDIN_DRY_RUN", "true").strip().lower() == "false"
            )
        # Bound by key (no value=): the widget's state lives in session_state
        # under "linkedin_live", so flipping it either way sticks.
        st.toggle(
            "Post to LinkedIn for real",
            key="linkedin_live",
            help=(
                "OFF = DRY RUN: the agent builds and logs the exact post but "
                "sends nothing (the safe default). ON = approving an item posts "
                "it live to the Metis page. Needs a LinkedIn token in .env -- "
        ...
        if st.session_state.linkedin_live:
            if os.getenv("LINKEDIN_ACCESS_TOKEN", "").strip():
                ...
                st.error(
                    "LIVE is on but no LINKEDIN_ACCESS_TOKEN in .env -- posting "
                    "will fail. See LINKEDIN_SETUP.md, or switch the toggle off."
                )
        else:
...
# --------------------------------------------------------------------------
# Page 1 -- Create (generate content)
# --------------------------------------------------------------------------
def page_create():
    ...
    st.caption(
        ...
        "guardrails and waits in the Review page -- nothing posts until you say so."
    )
    ...
    st.caption(
        "Draft a hot-topic reaction (best-of-N, voice + engagement scored) into "
        "the approval queue on the Review page. Nothing posts until you approve it."
    )
    ...
    if draft_btn and hot.strip():
        with st.spinner("Drafting best-of reaction and queuing..."):
            try:
                ...
                st.success("Queued %d item(s)%s. Open the Review page to approve."
                           % (len(res["queued"]), note))
            except SystemExit as exc:
                st.error(str(exc))
            except Exception as exc:  # noqa: BLE001
    ...
    if cycle_btn:
        with st.spinner("Scouting, ranking, drafting the best one..."):
            try:
                ...
                res = run_cycle.run()
                if res.get("queued"):
                    st.success("Reacted to '%s' and queued %d item(s). Open the Review page."
    ...
    st.caption(
        "Find a viral AI video and draft Metis commentary. The source link goes "
        "in the first comment (better reach); native re-upload stays disabled "
...
# --------------------------------------------------------------------------
# Page 2 -- Review (approve, browse, monitor)
# --------------------------------------------------------------------------
def page_review():
    st.title("Review Queue")
    st.caption(
        "Approve what the agents drafted, browse saved documents, and watch "
        "performance -- everything the system produced, in one place."
    )
    ...
    with tab_queue:
        ...
        st.caption(
            "Reactions waiting for approval. Edit the text right here, then "
        ...
        if not queued:
            st.info("Nothing queued. Use the Create page to draft a reaction.")
        for r in reversed(queued):
            # expanded=True so an item stays open while you work it: clicking any
            # button triggers a Streamlit rerun, and a collapsed expander would
            # hide the edit box (and any save confirmation) right after you click
            # Save, which reads as "editing did nothing."
            with st.expander(("[%s] %s" % (r["platform"], r["topic"]))[:100],
                ...
                with st.expander("Copy for manual posting"):
                    st.caption(
                        "Use the copy icon at the top-right of the box below, "
                        "then paste it into LinkedIn (or Substack) yourself. "
                        "Handy while auto-posting is off or the API is still "
                        "pending."
                    )
                ...
                with sv:
                    if st.button("Save edits", key="sv-%s" % r["id"], width="stretch"):
                        ...
                        # written it to the ledger, and skipping a second rerun
                        # lets the confirmation below stay on screen.
                ...
                with ap:
                    if st.button("Approve + post", key="ap-%s" % r["id"], width="stretch"):
                        # Save whatever is in the box first, so we post the
                        # edited version, then publish.
                        ...
                        if not saved["ok"]:
                            ...
                            try:
                                ...
                                # publisher raises this with a plain-English
                                # message (e.g. no token while live). Show it,
                                # do not crash.
    ...
    with tab_perf:
        ...
        st.caption(
            "What actually landed. Sync pulls real reactions/comments from "
            "LinkedIn into the ledger; the multipliers nudge which pillars the "
            "cycle favors."
        )
    ...
    with tab_trace:
        ...
        if not trace:
            ...
            # with "" (missing), which pyarrow cannot serialize into one column
            # and would crash st.dataframe. Strings render the same and always
            # convert.
...
# --------------------------------------------------------------------------
# App entry -- two pages in the sidebar nav
# --------------------------------------------------------------------------
