...
def _essay_due(history: list, interval_days: int = ESSAY_INTERVAL_DAYS) -> bool:
    """True if no essay has been published within the trailing interval, i.e.
    a quarterly essay is due. A fresh/empty history means one is due."""
...
def plan_cycle(scout_briefing: list = None, pillar_adjustments: dict = None) -> list:
    ...
    # Decide the slots for this cycle: essay first (if due), then field notes.
    slots = []
    if _essay_due(history):
        slots.append("essay")
    ...
    for i, fmt in enumerate(slots):
        ...
        plan.append({
            "slot": f"{label} {sum(1 for p in plan if p['format'] == fmt) + 1}",
            "format": fmt,
            "pillar": pillar,
