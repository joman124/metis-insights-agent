...
a single generate-evaluate-revise loop shared by the essay and field-note
writers.

This module is brand-agnostic: all the brand-specific data (banned phrases,
reference passages, the judge's system instruction) is imported from
voice_profile.py, so the same loop engine serves any voice.
"""
...
def find_antithesis(text: str) -> list:
    ...
    most persistent AI rhythm, so run_guardrails treats any hit as a hard
    fail -- unlike the softer NEGATIVE_PARALLELISM_FLAGS, which only flag for
    review."""
    hits = []
...
def run_guardrails(text: str, max_em_dashes: int = 2) -> dict:
    ...
    defaults to the field-note rule; pass a format's own
    CONTENT_RULES[...]['max_em_dashes'] for other formats (essays allow more)."""
    lowered = text.lower()
...
def judge_voice(text: str, model: str = None) -> dict:
    ...
    prompt = f"""REFERENCE PASSAGES (the firm's real voice):
{references}
...
def evaluate(text: str, max_em_dashes: int = 2, min_voice_score: int = 7,
             model: str = None) -> dict:
...
def draft_with_guardrails(model: str, build_prompt, system_instruction: str,
                           max_em_dashes: int = 2, max_attempts: int = 3,
    ...
    per-format sampling temperature for the draft calls (the judge inside
    evaluate() always runs deterministic regardless). Logs every attempt via
